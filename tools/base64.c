...
static int parse(const uint8_t *sec, int byteCount)
{
	...
	if (s) {
		...
		char *buf;
		uint32_t byteCount;
		int ret = scte35_create_base64_message(s, &buf, &byteCount);
		if (ret == 0) {
			printf("Base64 formatted message : ");
			printf("%s\n", buf);
			free(buf);
		} else {
			fprintf(stderr, "Unable to convert SCTE35 to Base64, ret = %d\n", ret);
		}
...
int scte35tobase64_main(int argc, char *argv[])
{
	int i = 0;
	int ret = 0;
	while (ret == 0) {
